...
module.exports = {
    ...
    updatePartById: function (partId, admin, data) {
        return Inventory.update({ admin: admin, _id: partId }, { $set: data }).exec();
    },
    delPartById: function (partId, admin) {
        return Inventory.update({ admin: admin, _id: partId }, { $set: {show : false}}).exec();
    },
    incPv: function incPv(partId) {
        return Inventory
            .update({ _id: partId }, { $inc: { pv: 1 } })
            .exec();
    },
    incInventory: function incInventory(partId) {
        return Inventory
            .update({ _id: partId }, { $inc: { inventory: 1 } })
            .exec();
    },
